...
import com.userplay.bazar22.databinding.FragmentJodiGroupBinding
...
@AndroidEntryPoint
class JodiGroupFragment : Fragment(R.layout.fragment_jodi_group), View.OnClickListener,
    OnGameTypeListener {

    @Inject
    lateinit var mPref: MatkaPref
    private var _binding: FragmentJodiGroupBinding? = null
    ...
    private val mArgs: JodiGroupFragmentArgs by navArgs()
    ...
    override fun onCreateView(
        ...
        _binding = FragmentJodiGroupBinding.inflate(inflater, container, false)
    ...
    private fun initView() {
        ...
        mBinding.apply {
            add.setOnClickListener(this@JodiGroupFragment)
            finalSubmit.setOnClickListener(this@JodiGroupFragment)
            back.setOnClickListener(this@JodiGroupFragment)
                ...
                when (v?.id) {
                    ...
                    R.id.add -> {
                        if (edNumber.text.isNullOrEmpty()) {
                            edNumber.error = "Jodi number can't be empty"
                            return@let
                        } else if (edNumber.text.length != 2) {
                            edNumber.error = "Please enter valid jodi"
                            return@let
                            ...
                            getJodiGroupNumbers(edNumber.text.toString().toInt()).forEach {item->
                                mGamesList.add(
                                    Game(
                                        edAmount.text.toString().trim().toInt(),
                                        if(item.toString().length==2){item.toString()}else{"0${item}"},
    ...
    private fun getJodiGroupNumbers(jodi: Int): List<Int> {
        ...
        return (firstSet + secondSet + thirdSet + fourthSet).distinct()
    }
}