...
import com.userplay.bazar22.databinding.FragmentFundWithdrawHistoryBinding
...
import com.userplay.bazar22.ui.fragments.funds.adapters.WithDrawFundAdapter
...
@AndroidEntryPoint
class FundWithdrawHistoryFragment : Fragment(R.layout.fragment_fund_withdraw_history),
    View.OnClickListener {

    private var _binding: FragmentFundWithdrawHistoryBinding? = null
    private val mBinding get() = _binding!!
    private val mList: ArrayList<Data> = ArrayList()
    private val mFundsViewModel: FundsViewModel by viewModels()
    private val mWithDrawFundAdapter: WithDrawFundAdapter by lazy {
        WithDrawFundAdapter(
            requireContext(),
            mList
        )
    ...
    override fun onCreateView(
        ...
        _binding = FragmentFundWithdrawHistoryBinding.inflate(inflater, container, false)
    ...
    private fun initView() {
        ...
        mBinding.apply {
            back.setOnClickListener(this@FundWithdrawHistoryFragment)
            btnNext.setOnClickListener(this@FundWithdrawHistoryFragment)
            btnPrevious.setOnClickListener(this@FundWithdrawHistoryFragment)
        }
        ...
        activity?.let {
            ...
            mBinding.recyclerView.apply {
                ...
                adapter = mWithDrawFundAdapter
            }

            if (CheckNetwork.isNetworkConnected) {
                mFundsViewModel.getWithdrawList(mCurrentPageNumber)
            } else {
    ...
    private fun observer() {
        activity?.let {
            ...
            mFundsViewModel.mGetWithDrawHistoryResponse.observe(it) { response ->
                when (response) {
                    ...
                    is ApiState.Success -> {
                        ...
                        if (response.data?.error != null) {
                            if (response.data.error) {
                                ...
                                mWithDrawFundAdapter.notifyDataSetChanged()
                            }
                        }
                    }
                    is ApiState.Error -> {
