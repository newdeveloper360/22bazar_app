...
import com.userplay.bazar22.databinding.FragmentJodiFamilyBinding
...
import com.userplay.bazar22.utils.generateJodiFamilyNumber2
import com.userplay.bazar22.utils.getJodiFamilyData
import dagger.hilt.android.AndroidEntryPoint
import javax.inject.Inject

@AndroidEntryPoint
class JodiFamilyFragment : Fragment(R.layout.fragment_jodi_family), View.OnClickListener,
    OnGameTypeListener {

    @Inject
    lateinit var mPref: MatkaPref
    private var _binding: FragmentJodiFamilyBinding? = null
    private val mBinding get() = _binding!!
    private val mGamesList: ArrayList<Game> = ArrayList()
    private var jodiFamilyList: ArrayList<NumberDataModel> = ArrayList()
    ...
    override fun onCreateView(
        ...
        _binding = FragmentJodiFamilyBinding.inflate(inflater, container, false)
        return mBinding.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        jodiFamilyList = getJodiFamilyData()
        initView()
    }
    ...
    private fun initView() {
        ...
        mBinding.apply {
            add.setOnClickListener(this@JodiFamilyFragment)
            finalSubmit.setOnClickListener(this@JodiFamilyFragment)
            back.setOnClickListener(this@JodiFamilyFragment)
                ...
                when (v?.id) {
                    ...
                    R.id.add -> {
                        if (edNumber.text.isNullOrEmpty()) {
                            ...
                            val item = jodiFamilyList.find { it.number == edNumber.text.toString() }
                            item?.let {
                                val listJodiFamilyNumber = it.number.generateJodiFamilyNumber2()
                                if (listJodiFamilyNumber.isEmpty()) {
                                    edNumber.error = "Invalid Jodi Number"
                                } else {
                                    listJodiFamilyNumber.forEach { item ->
                                        mGamesList.add(
                                            Game(
                                                edAmount.text.toString().trim().toInt(),
                                                item.takeIf { it.length > 1 } ?: "0${it}",
