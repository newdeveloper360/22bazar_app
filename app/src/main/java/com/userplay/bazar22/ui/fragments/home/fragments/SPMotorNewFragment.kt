...
import com.userplay.bazar22.databinding.FragmentSpMotorNewBinding
...
@AndroidEntryPoint
class SPMotorNewFragment : Fragment(R.layout.fragment_sp_motor_new), OnGameTypeListener  {
    private var _binding: FragmentSpMotorNewBinding? = null
    ...
    private val mArgs: SPMotorNewFragmentArgs by navArgs()
    ...
    var listPairNumbers:ArrayList<PairsModel> = getSpMotorNumbers("0")
    ...
    override fun onCreateView(
        ...
        _binding = FragmentSpMotorNewBinding.inflate(inflater, container, false)
    ...
    private fun initViews() {
        ...
        mBinding.btnSubmit.setOnClickListener {
            if (mGamesList.size == 0) {
                ...
                val action =
                    SPMotorNewFragmentDirections.actionGlobalSubmitGameDialogFragment(
    ...
    fun updatePairs(number:Int){
        mAdapterPair?.updateList(getSpMotorNumbers(number.toString()))
    }
    var lastSelected=0
    fun refreshAdapter(){
    ...
    private fun getSpMotorNumbers(selectedDigit: String): ArrayList<PairsModel> {
        val allDigits = ('0'..'9').toList() // Digits from 0 to 9
        val uniqueTrios = mutableListOf<String>()

        for (i in allDigits.indices) {
            for (j in i + 1 until allDigits.size) {
                if (selectedDigit != allDigits[i].toString() && selectedDigit != allDigits[j].toString()) continue

                for (k in j + 1 until allDigits.size) {
                    if (selectedDigit != allDigits[k].toString() && selectedDigit != allDigits[j].toString() && selectedDigit != allDigits[i].toString()) continue

                    val trio = listOf(allDigits[i], allDigits[j], allDigits[k]).sorted().joinToString("")
                    if (!uniqueTrios.contains(trio)) {
                        if (trio.startsWith("0")) {
                            uniqueTrios.add("${trio.replace("0", "")}0")
                        } else {
        ...
        return uniqueTrios.sorted().map { PairsModel(it.toInt()) }.toCollection(ArrayList())
    }
