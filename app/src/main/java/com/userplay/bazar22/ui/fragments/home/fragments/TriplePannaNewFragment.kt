...
import com.userplay.bazar22.databinding.FragmentTriplePannaNewBinding
...
import com.userplay.bazar22.utils.triplePanaBulkPairs
import dagger.hilt.android.AndroidEntryPoint
import javax.inject.Inject

@AndroidEntryPoint
class TriplePannaNewFragment : Fragment(R.layout.fragment_triple_panna_new), OnGameTypeListener {
    private var _binding: FragmentTriplePannaNewBinding? = null
    ...
    private val mArgs: TriplePannaNewFragmentArgs by navArgs()
    ...
    override fun onCreateView(
        ...
        _binding = FragmentTriplePannaNewBinding.inflate(inflater, container, false)
        return mBinding.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        listPairNumbers = 0.triplePanaBulkPairs()
        initViews()
        observer()
    }
    ...
    private fun initViews() {
        ...
        mBinding.btnAdd.setOnClickListener {
            ...
            find?.let {
                if (find.isNotEmpty()) {
                    find.forEach { item ->
                        //if pair number is "0" then add it as "000"
                        if (item.pairNumber.toString() == "0") {
                            mGamesList.add(
                                Game(
    ...
    fun updatePairs(number: Int) {
        mAdapterPair?.updateList(number.triplePanaBulkPairs())
    }
